...
class Graph():
  def __init__(self, nx_graphs, m_graph, p, q, r):
    self.G = nx_graphs
    self.p = p
    self.q = q
    self.r = r
  ...
  def simulate_walks(self, num_walks, walk_length):
    ...
    nodes = set([node for g in Gs  for node in g.nodes()])
    walks = []
    nodes = list(nodes)
    ...
    for walk_iter in range(num_walks):
      ...
      for node in nodes:
        if node not in G.nodes():
          G = [g for g in Gs if node in g.nodes()][0]
  ...
  def preprocess_transition_probs(self, weight_list, flag):
    ...
    for G in Gs:
      ...
      for node in G.nodes():
        ...
        jump_list = []  # The list of jump node between layers
        for graph in Gs:
          if node in graph.nodes():
            if graph == G: # 如果在该图中
              if flag == 0 or flag == 3:
                jump_list.append(self.r) # 留在该图中的概率
              else:
            ...
            jump_list.append(0) # 该图中没有指定的节点，那么就不跳转
